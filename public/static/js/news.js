	...
	$getJSON($URL_INDEX_NEWS+"/API.php?m=list&id="+num+"&p="+pageNo+"&s=10",function(data){
		...
		for(var i=0;i<data.new_list.length;i++){
			if(pageNo==1&&i==0){
				...
				news_msg+='<div class="news-focus">';
				var moren='\/dasai\/images\/news.jpg';
				news_msg+='<img src="'+$STATIC_URL+(data.new_list[i].imgsrc?data.new_list[i].imgsrc:moren)+'"/>';
			...
			news_msg+='<div class="block pl200">';
			var moren='\/dasai\/images\/news.jpg';
			news_msg+='<img src="'+$STATIC_URL+(data.new_list[i].imgsrc?data.new_list[i].imgsrc:moren)+'"/>';
			news_msg+='<div class="clearfix news-t">';
