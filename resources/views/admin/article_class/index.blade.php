...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mb-10">
        <span class="l">
            ...
            <a href="javascript:;" onclick="admin_permission_add('添加分类','/admin/article/class/add','','400')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加分类</a>
        </span>
    ...
    <table class="table table-border table-bordered table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="8">分类节点</th>
        </tr>
        <tr class="text-c">
        ...
        @foreach($data as $val)
            <tr class="text-c">
                ...
                <td>@if($val->link_url) {{ $val->link_url }} @else # @endif</td>
                <td>{{ $val->class_type }}</td>
                <td>@if($val->show_in_nav == '1')是 @else 否 @endif</td>
                <td>
                    <a title="编辑" href="javascript:;" onclick="admin_permission_edit('分类编辑','/admin/article/class/update/{{ $val->id }}','','400')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
function admin_permission_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
        $.ajax({
            type: 'POST',
            url: '/admin/article/class/delete?id='+id,
            dataType: 'json',
            data:{
                '_token':'{{ csrf_token() }}',
            },
